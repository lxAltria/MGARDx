...
int main(int argc, char ** argv)
{
  ...
  int rank = 0;
	...
	size_t est_compression_ratio = 30;
  ...
  if(rank == 0){
	...
	for(int i=0; i<num_vars; i++){
		...
		*/
		...
		MPI_Barrier(MPI_COMM_WORLD);
		if(rank == 0) elapsed_time = -MPI_Wtime();
		MGARD::Decomposer<float> decomposer(true);
		unsigned char * compressed = decomposer.compress(data, dims, target_level, abs_eb[i], compressed_size[i]); 
		MPI_Barrier(MPI_COMM_WORLD);
		if(rank == 0){
			elapsed_time += MPI_Wtime();
	...
	std::string zip_filename = folder + "/" + "mgard_compressed_" + std::to_string(rank) + ".dat";
  ...
  if(rank == 0){
	...
	zip_filename = folder + "/" + "mgard_compressed_" + std::to_string(size - 1 - rank) + ".dat";
	// Read Compressed Data
	unsigned char * compressed_input = (unsigned char *) malloc(total_size);
	MPI_Barrier(MPI_COMM_WORLD);
	if(rank == 0) elapsed_time = -MPI_Wtime();
	posix_read(zip_filename, compressed_input, total_size);
	MPI_Barrier(MPI_COMM_WORLD);
	if(rank == 0){
		elapsed_time += MPI_Wtime();
  ...
  if(rank == 0){
	...
	const unsigned char * compressed_input_pos = compressed_input;
	for(int i=0; i<num_vars; i++){
		// Decompress Compressed Data
		MPI_Barrier(MPI_COMM_WORLD);
		if(rank == 0) elapsed_time = -MPI_Wtime();
		MGARD::Recomposer<float> recomposer;
		float * dec_data = recomposer.decompress(compressed_input_pos, compressed_size[i], dims);
		MPI_Barrier(MPI_COMM_WORLD);
		if(rank == 0){
			elapsed_time += MPI_Wtime();
  ...
  if(rank == 0){
	...
	posix_delete(folder + "/" + "mgard_compressed_" + std::to_string(rank) + ".dat");
	if (rank == 0){
